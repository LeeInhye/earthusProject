<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>상품 수정 - Earth.Us</title>
<style>
	input::placeholder {font-size:small;}
</style>
</head>
<body>
	
	<%@ include file="../common/adminMenubar.jsp" %>
	
	<div id="layoutSidenav">
	
		<div id="layoutSidenav_content">
          <main>
              <div class="container-fluid px-4">
                  <div class="container insertProduct">
                      <h3 class="mt-4" style="font-weight:bold;">상품 > 등록</h3>
                      <hr><br>
                      <!-- 테이블 영역 시작-->
                      <form class="form-group" action="" method="post">
                          <div class="card mb-4" id="insert-pro-tb" style="border:none;">
                              <table class="insert-pro-tb">
                                  <tr>
                                      <td class="pro-info">
                                          * 카테고리 : 
                                      </td>
                                      <td class="pro-val">
                                          <select name="proCategory" id="proCategory">
                                              <option value="hair">Hair</option>
                                              <option value="body">Body</option>
                                              <option value="kitchen">Kitchen</option>
                                              <option value="bath">Bath</option>
                                          </select>
                                      </td>
                                      <td class="pro-btn"></td>
                                      <td width="45%"></td>
                                  </tr>

                                  <tr>
                                      <td class="pro-info">
                                          * 상품명 : 
                                      </td>
                                      <td colspan="2">
                                          <input type="text" class="form-control" name="proName" placeholder="상품명을 입력하세요.">
                                      </td>
                                  </tr>

                                  <tr>
                                      <td class="pro-info">상품코드 : </td>
                                      <td colspan="2">
                                          <input type="text" class="form-control" name="proCode" readonly>
                                      </td>
                                  </tr>

                                  <tr>
                                      <td class="pro-info">* 썸네일이미지 : </td>
                                      <td>
                                          파일원본명.확장자
                                      </td>
                                      <td>
                                          <input type="file" name="proImg" id="proImg" style="display:none;">
                                          <label for="proImg" class="btn-input">파일선택</label>
                                      </td>
                                  </tr>

                                  <tr>
                                      <td class="pro-info">* 상세이미지 : </td>
                                      <td>
                                          파일원본명.확장자
                                      </td>
                                      <td>
                                          <input type="file" name="proImg" id="proImg" style="display:none;">
                                          <label for="proImg" class="btn-input">파일선택</label>
                                      </td>
                                  </tr>

                                  <tr>
                                      <td class="pro-info">
                                          * 판매가격 : 
                                      </td>
                                      <td colspan="2">
                                          <input type="number" class="form-control" name="proName" placeholder="판매 가격을 입력하세요. 단위 : 원">
                                      </td>
                                  </tr>

                                  <tr>
                                      <td class="pro-info">
                                          * 재고수량 : 
                                      </td>
                                      <td class="pro-val">
                                          <input type="number" class="form-control" name="stock" id="stock">
                                      </td>
                                      <td class="pro-btn"></td>
                                  </tr>

                                  <tr>
                                      <td class="pro-info">* 필수표기정보 : </td>
                                      <td>
                                          파일원본명.확장자
                                      </td>
                                      <td>
                                          <input type="file" name="proImg" id="proImg" style="display:none;">
                                          <label for="proImg" class="btn-input">파일선택</label>
                                      </td>
                                  </tr>
                              </table>
                          </div>

                          <div id="enroll-btn-area">
                              <input type="submit" class="btn-input" value="등록" style="border:none;" onclick="proEnroll();">
                              <a href="" class="btn btn-sm btn-input" style="background:#f2f2f2; color:black; vertical-align:middle;">취소</a>
                          </div>
                      </form>
                      <!-- 테이블 영역 끝-->
                  </div>
              </div>
          </main>
	</div>
      <script>
          function proEnroll(){
              
             if(confirm("상품을 등록하시겠습니까?")){
                  alert("등록 완료되었습니다.");
                  location.href = "";
             }else {
                  // 기존 등록 페이지로 돌아감
             }

          }
      </script>
</body>
</html>